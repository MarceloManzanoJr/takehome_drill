...
def create_book():
    ...
    if not isinstance(data["year"], int):
        return jsonify({"success": False, "error": "The 'year' field must be an integer."}), HTTPStatus.BAD_REQUEST
...
def update_book(book_id):
    ...
    data = request.get_json()
    if not data:
        return jsonify({"success": False, "error": "No data found. Provide data to update the book."}), HTTPStatus.BAD_REQUEST
...
def internal_server_error(error):
    ...
    return jsonify({"success": False, "error": "An unexpected error occurred. Please try again later."}), HTTPStatus.INTERNAL_SERVER_ERROR
